...
import { batchConfirmMatches } from '@/lib/services/matching'
...
export async function POST(
  request: Request
): Promise<NextResponse<BatchConfirmResponse | { error: string }>> {
  try {
    // Authenticate user
    ...
    // Parse request body
    const body = (await request.json()) as BatchConfirmRequest
    ...
    // Confirm matches
    const { data, error } = await batchConfirmMatches(body.match_ids, user.id)
    ...
    console.error('Error in POST /api/trophy/batch-confirm:', error)
